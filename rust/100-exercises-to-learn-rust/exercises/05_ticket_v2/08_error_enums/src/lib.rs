...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::DescriptionParseError(_)) => {
            Ticket::new(title, "Description not provided".to_string(), status).unwrap()
        }
        Err(TicketNewError::TitleParseError(msg)) => panic!("{}", msg),
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleParseError(
        ...
        if description.is_empty() {
            return Err(TicketNewError::DescriptionParseError(
